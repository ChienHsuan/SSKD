...
class MSABlock_1(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.gate = ContextBlock(mid_channels, 1/16)
...
class MSANet(nn.Module):
    ...
    def _make_layer(
        ...
        layers.append(block[0](in_channels, out_channels, IN=IN[0]))
        for i in range(1, layer):
            layers.append(block[i](out_channels, out_channels, IN=IN[i]))
...
##########
def msanet(
    num_features=512, num_classes=1000, **kwargs
):
    model = MSANet(
        num_classes,
